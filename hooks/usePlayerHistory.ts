...
import type { HistoryChartModel, HistoryIntervalId, HistoryRangeId } from '@/lib/playerHistory';
import {
...
import { isPlayersHistoryPayload } from '@/lib/types';
...
export function usePlayerHistory(options: UsePlayerHistoryOptions): UsePlayerHistoryResult {
  ...
  const updateSelectedRangeId = useCallback((nextRangeId: HistoryRangeId) => {
    ...
    setSelectedIntervalId((currentIntervalId) =>
      nextAllowedIntervals.some((interval) => interval.id === currentIntervalId)
        ? currentIntervalId
